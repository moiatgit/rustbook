...
<span class="caption">Llistat 15-1: Storing an `i32` value on the heap using a
box</span>
...
<span class="caption">Llistat 15-2: The first attempt at defining an enum to
...
<span class="caption">Llistat 15-3: Using the `List` enum to store the list `1,
2, 3`</span>
...
<span class="caption">Llistat 15-4: The error we get when attempting to define
...
<span class="caption">Llistat 15-5: Definition of `List` that uses `Box<T>` in
